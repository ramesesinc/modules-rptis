...
from subdivision s
	inner join subdivision_motherland ml on s.objid = ml.subdivisionid
	inner join faas mf on ml.landfaasid = mf.objid 
	inner join subdividedland sl on s.objid = sl.subdivisionid
	inner join faas slf on sl.newfaasid = slf.objid 
where s.objid = $P{objid}
