...
public abstract class AbstractBatchGRModel extends RPTWorkflowController
{
    ...
    public void afterSignal(Object result){
        ...
        if (task && task.state.matches('muniapprover|cityapprover|provapprover|muniapprover|forprovsubmission')){
            autoposting = true;
        }
    }
    
    public Object findPage(Map o){
         if (entity.state != 'APPROVED' && task && task.state.matches('muniapprover|cityapprover|provapprover|muniapprover|forprovsubmission')){
             info = null;
             if (autoposting)
                doApprove();
             return 'wait';
